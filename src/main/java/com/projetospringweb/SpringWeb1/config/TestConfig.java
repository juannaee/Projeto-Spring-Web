...
public class TestConfig implements CommandLineRunner {
	...
	public void run(String... args) {
		...
		CategoryProduct cp2 = new CategoryProduct(null, "Eletronicos");
		...
		Product p7 = new Product(null, "Monitor UltraWide", "Monitor de alta resolução com tela ampla", 1899.00,
				"monitorultrawide.com.br");

		productRepository.saveAll(Arrays.asList(p1, p2, p3, p4, p5, p6, p7));
		categoryRepository.saveAll(Arrays.asList(cp1, cp2));

		p1.getCategories().add(cp1);
		p1.getCategories().add(cp2);
		p2.getCategories().add(cp1);
		p3.getCategories().add(cp1);
		p4.getCategories().add(cp1);
		p5.getCategories().add(cp1);
		p6.getCategories().add(cp1);
		p7.getCategories().add(cp1);
